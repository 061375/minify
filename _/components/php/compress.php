...
class Compress {
    ...
    {
        $this->devip = isset($params['devip']) ? $params['devip'] : array('::1');
        ...
        if(!isset($_SERVER['REMOTE_ADDR']))die('<META NAME="ROBOTS" CONTENT="NOINDEX, NOFOLLOW" data-er="1" />');
        if(!in_array($_SERVER['REMOTE_ADDR'],$this->devip))die('<META NAME="ROBOTS" CONTENT="NOINDEX, NOFOLLOW" data-er="2" />');
        
        $this->isdev = true;
        
        $this->filepath = isset($params['filepath']) ? $params['filepath'] : false;
        
        $this->workurl = isset($params['workurl']) ? $params['workurl'] : false;
        
        if(false === $this->filepath OR false === $this->workurl)
            die('filepath and workurl are required '.__METHOD__.'-'.__LINE__);
    }
    /**
    ...
    function script($wpath,$vpath,$script,$other = false)
    {
        // if before compress
        if(!isset($_POST['chgfysfuygf54fg'])) {
            // there are libraries, they should be loaded first
            if(false !== $other) {
                foreach($other as $o) {
